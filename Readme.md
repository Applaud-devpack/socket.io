...
@applaud/socket.io enables real-time bidirectional event-based communication. It consists of:
...
In order to create separation of concerns within your application (for example per module, or based on permissions), @applaud/socket.io allows you to create several `Namespaces`, which will act as separate communication channels but will share the same underlying connection.
...
**Note:** @applaud/socket.io is not a WebSocket implementation. Although @applaud/socket.io indeed uses WebSocket as a transport when possible, it adds some metadata to each packet: the packet type, the namespace and the ack id when a message acknowledgement is needed. That is why a WebSocket client will not be able to successfully connect to a @applaud/socket.io server, and a @applaud/socket.io client will not be able to connect to a WebSocket server (like `ws://echo.websocket.org`) either. Please see the protocol specification [here](https://github.com/socketio/socket.io-protocol).

## Installation

```bash
// with npm
npm install @applaud/socket.io --save-dev

// with yarn
yarn add @applaud/socket.io
```

## How to use

The following example attaches @applaud/socket.io to a plain Node.JS
HTTP server listening on port `3000`.

```js
const server = require('http').createServer();
const io = require('@applaud/socket.io')(server);
...
```js
const io = require('@applaud/socket.io')();
io.on('connection', client => { ... });
io.listen(3000);
```
...
```js
import { Server } from "@applaud/socket.io";
...
To integrate @applaud/socket.io in your Fastify application you just need to
...
@applaud/socket.io is powered by [debug](https://github.com/visionmedia/debug).
In order to see all the debug output, run your app with the environment variable
`DEBUG` including the desired scope.

To see the output from all of @applaud/socket.io's debugging scopes you can use:
